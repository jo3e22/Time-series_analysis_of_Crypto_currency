...
saveDir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../savedFigs/'))
...
def process_and_merge_data(data_array, max_rows=None):
    """Process and merge cryptocurrency data for analysis"""
    processed_dfs = []
...
def calculate_time_lag(series1, series2, max_lag=100):
    """Calculate time lag between two time series using cross-correlation"""
...
def introduce_gaps(series, gap_percentage, periodic=True):
    """Introduce gaps in time series data"""
...
def analyze_gap_impact(df, columns, gap_percentages=[0.05, 0.1, 0.2, 0.3]):
    """Analyze how data gaps affect correlation and lag detection"""
    results = {}
...
def plot_gap_impact(gap_results, metric='corr'):
    """Plot impact of gaps on correlation or lag detection"""
...
def bootstrap_lag_confidence(series1, series2, n_bootstrap=1000, confidence=0.95):
    """Use bootstrap to calculate confidence intervals for time lag estimates"""
    n = len(series1)
    lag_results = []
...
if __name__ == "__main__":
    results = analyze_crypto_time_series(max_rows=15000, lambda_=10)
...
#%% Additional analysis for the original plotting function
def run_resample_and_plot(lambda_=50):
    """Run the original plotting function"""
    resample_and_plot(arr_o_data, lambda_)
...
def resample_and_plot(dfs, lambda_=10):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(saveDir, 'fft_analysis.png'), dpi=300)
    plt.show()